...
class DataTransformation:
    #taking input and output variable referance from config_entity.py and artifact_entity.py file
    ...
    #so we start data transformation on training data and test data
    #we have to access or read train and test data using path variable stored in artifact_entity.py path
    def initiate_data_transformation(self,) -> artifact_entity.DataTransformationArtifact:
        try:
            #reading trian and test data
            train_df = pd.read_csv(self.data_ingestion_artifact.train_file_path)
            test_df = pd.read_csv(self.data_ingestion_artifact.test_file_path)
            
            #creating independent data i.e. x_train and x_test by
            #removing target column from train and test data by using target column variable saved in config.py file
            input_feature_train_df=train_df.drop(TARGET_COLUMN,axis=1)
            input_feature_test_df=test_df.drop(TARGET_COLUMN,axis=1)

            #creating dependent data i.e. y_train and y_test 
            target_feature_train_df = train_df[TARGET_COLUMN]
            target_feature_test_df = test_df[TARGET_COLUMN]

            label_encoder = LabelEncoder()

            #coverting x_test and y_test column into scaler
